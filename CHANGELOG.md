...
- *(security)* `ECPHORY_AUTH_TOKEN` now gates the MCP transport as well as the REST mirror. `require_auth` was layered on `/api/v1` inside `build_router` while `serve_http` nested `/mcp` onto the finished router, outside it, so setting a token produced a gated REST plane and a wide-open tool surface: no session id required, all 11 tools listed, and an unauthenticated `add_memory` landing an episode on a store whose `/api/v1` had just answered the same caller `401`. `build_router` now takes the transport as a parameter and nests it beside the REST plane under one layer, so a surface is mounted inside the gate or not at all. Inert unless a token is set, so a daemon that never opted in is unaffected (#47)

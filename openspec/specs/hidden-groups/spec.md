...
## Purpose
Let a store hold groups that should not surface in general recall — a todo queue, a
noisy import — without deleting them or splitting the store. Unscoped search skips a
hidden group; naming the group is the opt-in that returns it.
## Requirements

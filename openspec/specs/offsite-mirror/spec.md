...
## Purpose
Carry the mirror's cold tier off the machine automatically, so a committed export
survives losing the disk. ecphory pushes to a git remote it already has and fires a
store-wide `export` event for any other destination, staying out of the credential
business. Offsite is best-effort: it never compromises the local export.
## Requirements
